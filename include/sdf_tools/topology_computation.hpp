...
namespace topology_computation
{
    ...
    int64_t MarkConnectedComponent(const VoxelGrid::VoxelGrid<T, Allocator, BackingStore>& source_grid,
                                   const std::function<bool(const VoxelGrid::GRID_INDEX&, const VoxelGrid::GRID_INDEX&)>& are_connected_fn,
                                   const std::function<uint32_t(const VoxelGrid::GRID_INDEX&)>& get_component_fn,
                                   const std::function<void(const VoxelGrid::GRID_INDEX&, const uint32_t)>& mark_component_fn,
                                   int64_t x_index, int64_t y_index, int64_t z_index, uint32_t connected_component)
    {
        ...
        const size_t queued_hashtable_size_hint = source_grid.GetImmutableRawData().size() / 32;
        ...
        while (working_queue.size() > 0)
        {
            ...
            const VoxelGrid::GRID_INDEX current_index = working_queue.front();
            ...
            const std::vector<VoxelGrid::GRID_INDEX> neighbor_indices = {VoxelGrid::GRID_INDEX(current_index.x - 1, current_index.y, current_index.z),
                                                                         VoxelGrid::GRID_INDEX(current_index.x + 1, current_index.y, current_index.z),
                                                                         VoxelGrid::GRID_INDEX(current_index.x, current_index.y - 1, current_index.z),
                                                                         VoxelGrid::GRID_INDEX(current_index.x, current_index.y + 1, current_index.z),
                                                                         VoxelGrid::GRID_INDEX(current_index.x, current_index.y, current_index.z - 1),
                                                                         VoxelGrid::GRID_INDEX(current_index.x, current_index.y, current_index.z + 1)};
            for (size_t idx = 0; idx < neighbor_indices.size(); idx++)
            {
                const VoxelGrid::GRID_INDEX& neighbor_index = neighbor_indices[idx];
    ...
    template<typename T, typename Allocator=std::allocator<T>, typename BackingStore=std::vector<T, Allocator>>
    uint32_t ComputeConnectedComponents(const VoxelGrid::VoxelGrid<T, Allocator, BackingStore>& source_grid,
                                        const std::function<bool(const VoxelGrid::GRID_INDEX&, const VoxelGrid::GRID_INDEX&)>& are_connected_fn,
                                        const std::function<int64_t(const VoxelGrid::GRID_INDEX&)>& get_component_fn,
                                        const std::function<void(const VoxelGrid::GRID_INDEX&, const uint32_t)>& mark_component_fn)
    {
        ...
        for (int64_t x_index = 0; x_index < source_grid.GetNumXCells(); x_index++)
        {
            for (int64_t y_index = 0; y_index < source_grid.GetNumYCells(); y_index++)
            {
                ...
                {
                    const VoxelGrid::GRID_INDEX index(x_index, y_index, z_index);
                    mark_component_fn(index, 0u);
                }
            }
        }
        // Mark the components
        int64_t total_cells = source_grid.GetNumXCells() * source_grid.GetNumYCells() * source_grid.GetNumZCells();
        int64_t marked_cells = 0;
        ...
        for (int64_t x_index = 0; x_index < source_grid.GetNumXCells(); x_index++)
        {
            for (int64_t y_index = 0; y_index < source_grid.GetNumYCells(); y_index++)
            {
                for (int64_t z_index = 0; z_index < source_grid.GetNumZCells(); z_index++)
                {
                    ...
                    int64_t cells_marked = MarkConnectedComponent(source_grid, are_connected_fn, get_component_fn, mark_component_fn, x_index, y_index, z_index, connected_components);
    ...
    inline int32_t ComputeConnectivityOfSurfaceVertices(const std::unordered_map<VoxelGrid::GRID_INDEX, uint8_t>& surface_vertex_connectivity)
    {
        ...
        {
            const VoxelGrid::GRID_INDEX key = surface_vertices_itr->first;
            ...
            else
            {
                ...
                while (working_queue.size() > 0)
                {
                    ...
                    const VoxelGrid::GRID_INDEX current_vertex = working_queue.front();
                    working_queue.pop_front();
                    ...
                    if ((connectivity & 0b00000001) > 0)
                    {
                        ...
                        const VoxelGrid::GRID_INDEX connected_vertex(current_vertex.x, current_vertex.y, current_vertex.z - 1);
                    ...
                    {
                        // Try to add the vertex
                        const VoxelGrid::GRID_INDEX connected_vertex(current_vertex.x, current_vertex.y - 1, current_vertex.z);
                        // We only add if we haven't already processed it
                        if (queued_hashtable[connected_vertex] <= 0)
                        {
                            queued_hashtable[connected_vertex] = 1;
                    ...
                    {
                        // Try to add the vertex
                        const VoxelGrid::GRID_INDEX connected_vertex(current_vertex.x - 1, current_vertex.y, current_vertex.z);
                        // We only add if we haven't already processed it
                        if (queued_hashtable[connected_vertex] <= 0)
                        {
                            queued_hashtable[connected_vertex] = 1;
    ...
    template<typename T, typename Allocator=std::allocator<T>, typename BackingStore=std::vector<T, Allocator>>
    std::map<uint32_t, std::unordered_map<VoxelGrid::GRID_INDEX, uint8_t>> ExtractComponentSurfaces(const VoxelGrid::VoxelGrid<T, Allocator, BackingStore>& source_grid,
                                                                                                    const std::function<int64_t(const VoxelGrid::GRID_INDEX&)>& get_component_fn,
                                                                                                    const std::function<bool(const VoxelGrid::GRID_INDEX&)>& is_surface_index_fn)
    {
        ...
        for (int64_t x_index = 0; x_index < source_grid.GetNumXCells(); x_index++)
        {
            for (int64_t y_index = 0; y_index < source_grid.GetNumYCells(); y_index++)
            {
                for (int64_t z_index = 0; z_index < source_grid.GetNumZCells(); z_index++)
                {
                    ...
                    {
                        component_surfaces[get_component_fn(current_index)][current_index] = 1;
                    }
                }
            }
        }
    ...
    inline std::pair<int32_t, int32_t> ComputeHolesInSurface(const uint32_t component,
                                                             const std::unordered_map<VoxelGrid::GRID_INDEX, uint8_t>& surface,
                                                             const std::function<int64_t(const VoxelGrid::GRID_INDEX&)>& get_component_fn,
                                                             const bool verbose)
    {
        ...
        for (auto surface_itr = surface.begin(); surface_itr != surface.end(); ++surface_itr)
        {
            ...
            const int64_t xyzm1_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x, current_index.y, current_index.z - 1));
            const int64_t xyzp1_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x, current_index.y, current_index.z - 1));
            const int64_t xym1z_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x, current_index.y - 1, current_index.z));
            const int64_t xyp1z_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x, current_index.y + 1, current_index.z));
            const int64_t xm1yz_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x - 1, current_index.y, current_index.z));
            const int64_t xp1yz_component = get_component_fn(VoxelGrid::GRID_INDEX(current_index.x + 1, current_index.y, current_index.z));
            ...
            if (component != xyzm1_component || component != xym1z_component || component != xm1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex1(current_index.x, current_index.y, current_index.z);
                surface_vertices[vertex1] = 1;
            }
            // Second, check the (-,-,+) vertex
            if (component != xyzp1_component || component != xym1z_component || component != xm1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex2(current_index.x, current_index.y, current_index.z + 1);
                surface_vertices[vertex2] = 1;
            }
            // Third, check the (-,+,-) vertex
            if (component != xyzm1_component || component != xyp1z_component || component != xm1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex3(current_index.x, current_index.y + 1, current_index.z);
                surface_vertices[vertex3] = 1;
            }
            // Fourth, check the (-,+,+) vertex
            if (component != xyzp1_component || component != xyp1z_component || component != xm1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex4(current_index.x, current_index.y + 1, current_index.z + 1);
                surface_vertices[vertex4] = 1;
            }
            // Fifth, check the (+,-,-) vertex
            if (component != xyzm1_component || component != xym1z_component || component != xp1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex5(current_index.x + 1, current_index.y, current_index.z);
                surface_vertices[vertex5] = 1;
            }
            // Sixth, check the (+,-,+) vertex
            if (component != xyzp1_component || component != xym1z_component || component != xp1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex6(current_index.x + 1, current_index.y, current_index.z + 1);
                surface_vertices[vertex6] = 1;
            }
            // Seventh, check the (+,+,-) vertex
            if (component != xyzm1_component || component != xyp1z_component || component != xp1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex7(current_index.x + 1, current_index.y + 1, current_index.z);
                surface_vertices[vertex7] = 1;
            }
            // Eighth, check the (+,+,+) vertex
            if (component != xyzp1_component || component != xyp1z_component || component != xp1yz_component)
            {
                const VoxelGrid::GRID_INDEX vertex8(current_index.x + 1, current_index.y + 1, current_index.z + 1);
        ...
        for (surface_vertices_itr = surface_vertices.begin(); surface_vertices_itr != surface_vertices.end(); ++surface_vertices_itr)
        {
            ...
            const int64_t xm1ym1zm1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x - 1, key.y - 1, key.z - 1));
            const int64_t xm1ym1zp1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x - 1, key.y - 1, key.z + 0));
            const int64_t xm1yp1zm1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x - 1, key.y + 0, key.z - 1));
            const int64_t xm1yp1zp1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x - 1, key.y + 0, key.z + 0));
            const int64_t xp1ym1zm1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x + 0, key.y - 1, key.z - 1));
            const int64_t xp1ym1zp1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x + 0, key.y - 1, key.z + 0));
            const int64_t xp1yp1zm1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x + 0, key.y + 0, key.z - 1));
            const int64_t xp1yp1zp1_component = get_component_fn(VoxelGrid::GRID_INDEX(key.x + 0, key.y + 0, key.z + 0));
            // Check the "z- down" edge
            if (component != xm1ym1zm1_component || component != xm1yp1zm1_component || component != xp1ym1zm1_component || component != xp1yp1zm1_component)
            {
                if (!(component != xm1ym1zm1_component && component != xm1yp1zm1_component && component != xp1ym1zm1_component && component != xp1yp1zm1_component))
                {
                    edge_count++;
            ...
            // Check the "z+ up" edge
            if (component != xm1ym1zp1_component || component != xm1yp1zp1_component || component != xp1ym1zp1_component || component != xp1yp1zp1_component)
            {
                if (!(component != xm1ym1zp1_component && component != xm1yp1zp1_component && component != xp1ym1zp1_component && component != xp1yp1zp1_component))
                {
                    edge_count++;
            ...
            if (component != xm1ym1zm1_component || component != xm1ym1zp1_component || component != xp1ym1zm1_component || component != xp1ym1zp1_component)
            {
                if (!(component != xm1ym1zm1_component && component != xm1ym1zp1_component && component != xp1ym1zm1_component && component != xp1ym1zp1_component))
                {
                    edge_count++;
            ...
            if (component != xm1yp1zm1_component || component != xm1yp1zp1_component || component != xp1yp1zm1_component || component != xp1yp1zp1_component)
            {
                if (!(component != xm1yp1zm1_component && component != xm1yp1zp1_component && component != xp1yp1zm1_component && component != xp1yp1zp1_component))
                {
                    edge_count++;
            ...
            if (component != xm1ym1zm1_component || component != xm1ym1zp1_component || component != xm1yp1zm1_component || component != xm1yp1zp1_component)
            {
                if (!(component != xm1ym1zm1_component && component != xm1ym1zp1_component && component != xm1yp1zm1_component && component != xm1yp1zp1_component))
                {
                    edge_count++;
            ...
            if (component != xp1ym1zm1_component || component != xp1ym1zp1_component || component != xp1yp1zm1_component || component != xp1yp1zp1_component)
            {
                if (!(component != xp1ym1zm1_component && component != xp1ym1zp1_component && component != xp1yp1zm1_component && component != xp1yp1zp1_component))
                {
                    edge_count++;
    ...
    template<typename T, typename Allocator=std::allocator<T>, typename BackingStore=std::vector<T, Allocator>>
    std::map<uint32_t, std::pair<int32_t, int32_t>> ComputeComponentTopology(const VoxelGrid::VoxelGrid<T, Allocator, BackingStore>& source_grid,
                                                                             const std::function<int64_t(const VoxelGrid::GRID_INDEX&)>& get_component_fn,
                                                                             const std::function<bool(const VoxelGrid::GRID_INDEX&)>& is_surface_index_fn,
                                                                             const bool verbose)
    {
        // Extract the surfaces of each connected component
        const std::map<uint32_t, std::unordered_map<VoxelGrid::GRID_INDEX, uint8_t>> component_surfaces = ExtractComponentSurfaces(source_grid, get_component_fn, is_surface_index_fn);
        // Compute the number of holes in each surface
        std::map<uint32_t, std::pair<int32_t, int32_t>> component_holes_and_voids;
        for (auto component_surfaces_itr = component_surfaces.begin(); component_surfaces_itr != component_surfaces.end(); ++component_surfaces_itr)
        {
            const uint32_t component_number = component_surfaces_itr->first;
            const std::unordered_map<VoxelGrid::GRID_INDEX, uint8_t>& component_surface = component_surfaces_itr->second;
            const std::pair<int32_t, int32_t> number_of_holes_and_voids = ComputeHolesInSurface(component_number, component_surface, get_component_fn, verbose);
